...
class HiringGraphBuilder:
    ...
    def build_graph(self):
        ...
        # 5. Add company nodes
        # ────────────────────────────────────────────────────────
        # FIX: Previously, company edges were assigned using
        # hiring_score, university_tier, technical_score, and
        # selected — i.e. the SAME signals the GNN is trying to
        # predict / the SAME features already used elsewhere in
        # the model. That made any "Company X is biased toward
        # Y" finding circular (the answer was leaking into the
        # graph structure itself).
        #
        # NEW APPROACH: Companies are defined by which
        # SPECIALIZATION cluster they recruit from — a genuine,
        # independent CV attribute that is NOT used as a GNN
        # training target and not part of the bias-factor
        # feature set. Within a company's specialization pool,
        # a deterministic hash of candidate_id decides whether
        # that candidate was "hired" by that company (simulating
        # limited open positions) — this hash carries NO
        # information about hiring_score, tier, or test scores.
        ...
        # Map each specialization to one company (round-robin)
        specializations = sorted(self.df['specialization'].dropna().unique())
        spec_to_company = {
            spec: companies[i % len(companies)]
        ...
        for idx, row in self.df.iterrows():
            ...
            if pd.isna(spec) or spec not in spec_to_company:
                continue

            target_company = spec_to_company[spec]

            # Deterministic hash of candidate_id — independent of
            # hiring_score / tier / test scores. Roughly 1 in 3
            # candidates in a company's specialization pool get
            # a "hired" edge (simulates limited open positions).
            h = int(hashlib.md5(str(cand_id).encode()).hexdigest(), 16)
            if h % 3 == 0:
                self.G.add_edge(target_company, cand_id, relation='hired',
                                score=hiring_score)
...
if __name__ == "__main__":
    dataset_path = 'resume_dataset_augmented.csv'
